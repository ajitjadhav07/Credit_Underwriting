...
class CamEligibility {
    ...
    deriveDefaults(extractedData) {
        ...
        // Prefill Banking Program ADB fields from Novel Pattern download results
        // novel_download.accounts[] has one entry per bank statement,
        // each with adb_lakhs already converted to lakhs.
        const novelDl = ed.external_verification?.novel_download || ed.novel_download;
        if (novelDl && Array.isArray(novelDl.accounts) && novelDl.accounts.length > 0) {
            const accts = novelDl.accounts;
            if (accts[0]) {
                base.adb_account_1 = accts[0].adb_lakhs;
                base.avg_monthly_credits = accts[0].avg_receipt_12m;
            }
            if (accts[1]) base.adb_account_2 = accts[1].adb_lakhs;
            if (accts[2]) base.adb_account_3 = accts[2].adb_lakhs;
            // Bounce check: inward returns > 0 for any account
            const totalBounces = accts.reduce((s, a) => s + (a.inward_return_count || 0), 0);
            base._novel_inward_bounces = totalBounces;
        }
