...
class CalculationEngine {
    ...
    calculateAll(extractedData) {
        ...
        // CIBIL data fetched via Commercial CIBIL (external-apis-manager.js
        // -> fetchCommercialCIBIL()). NOTE: the cibil_score/dpd_history field
        // paths inside that response are NOT YET VERIFIED against a real
        // UAT response — see NEEDS_REAL_RESPONSE_SAMPLE markers there.
        const cibilData = extractedData.external_verification?.cibil_commercial?.success
        ...
        return {
            ...
            credit_score: this.calcCreditScore(bsFY25, pnlFY25, pnlFY24, bankData, gstData, kycData, propertyData, cibilData),
    ...
    calcCreditScore(bs, pnlFY25, pnlFY24, bankData, gstData, kycData, propertyData, cibilData) {
        const ca = bs.current_assets || 0;
        ...
        // Credit History (15 points) — CIBIL score + DPD history via NSDL/CIBIL
        // Middleware integration (external-apis-manager.js -> fetchCIBIL()).
        let ch;
        if (cibilData && cibilData.cibil_score !== null && cibilData.cibil_score !== undefined) {
            const cibilScoreEval = this.evaluateMetric(cibilData.cibil_score, 'CIBIL_SCORE');
            const dpdEval = this.evaluateMetric(cibilData.max_dpd_last_12m ?? 0, 'DPD_HISTORY');

            ch = {
                name: 'Credit History', max: chMax,
                breakdown: [
                    {
                        metric: 'CIBIL Score',
                        value: cibilData.cibil_score,
                        score: cibilScoreEval.score,
                        max: cibilScoreEval.max,
                        logic: cibilScoreEval.logic
                    },
                    {
                        metric: 'DPD History (max, 12m)',
                        value: `${cibilData.max_dpd_last_12m ?? 0} days`,
                        score: dpdEval.score,
                        max: dpdEval.max,
                        logic: dpdEval.logic
                    }
                ],
                source: 'CIBIL via Middleware Gateway',
                report_date: cibilData.report_date || null
            };
            ch.score = ch.breakdown.reduce((s, b) => s + b.score, 0);
        } else {
            const reason = cibilData === null
                ? 'CIBIL not fetched — PAN unavailable or external verification not configured'
                : (cibilData?.error || 'Bureau API not integrated');
            ch = { name: 'Credit History', max: chMax, score: 0, reason,
                breakdown: [
                    { metric: 'CIBIL Score', value: 'N/A', score: 0, max: 8, reason },
                    { metric: 'DPD History (max, 12m)', value: 'N/A', score: 0, max: 7, reason }
                ]
            };
        }
