    ...
    async _flush(bufferName) {
        ...
        try {
            if (this.config.s3Enabled && this.s3?.isConfigured?.()) {
                ...
                // s3-client.js exports uploadFile(key, buffer, contentType) — v3 SDK.
                // The old putObject() call was AWS SDK v2 style and doesn't exist here.
                await this.s3.uploadFile(
                    key,
                    Buffer.from(JSON.stringify(eventsToFlush, null, 2)),
                    'application/json'
                );
