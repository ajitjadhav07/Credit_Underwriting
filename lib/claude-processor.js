 ...
 * Handles document extraction using Anthropic API on the server
 * 
...
class ClaudeProcessor {
    ...
    initialize() {
        ...
        try {
            ...
            console.log('✅ [CLAUDE] Processor initialized with API key');
            return true;
        } catch (err) {
            console.error('❌ [CLAUDE] Failed to initialize client:', err.message);
            return false;
        }
    }
    ...
    async checkHealth() {
        ...
        if (!this.initialized || !this.client) {
            return {
                working: false,
                configured: true,
                details: 'API key configured but client not initialized'
            };
        }
        ...
        try {
            ...
            // Make a minimal API call (just count tokens, doesn't generate)
            ...
            return {
                working: true,
                configured: true,
                details: `Claude API connected successfully (${latencyMs}ms latency)`,
                latencyMs,
            ...
            if (error.status === 401 || error.message.includes('401') || error.message.includes('Unauthorized')) {
                errorDetail = 'Authentication failed - API key is invalid or expired';
            } else if (error.status === 403 || error.message.includes('403')) {
                errorDetail = 'Access forbidden - API key may lack permissions';
            } else if (error.status === 429 || error.message.includes('rate')) {
                errorDetail = 'Rate limited - too many requests (but API is working)';
                // Rate limit means the API is actually working
                return {
                    working: true,
                    configured: true,
                    details: 'Claude API connected (rate limited but functional)',
                    rateLimited: true
                };
            } else if (error.message.includes('ECONNREFUSED') || error.message.includes('ENOTFOUND')) {
                errorDetail = 'Network error - cannot reach Anthropic API';
            } else if (error.message.includes('timeout')) {
                errorDetail = 'Connection timeout - Anthropic API may be slow';
            }
