 ...
 * Wraps document extraction with Google Vision API for scanned documents
...
async function processDocument(pdfBuffer, documentName, progressCallback = null) {
    ...
    const visionConfig = visionOcr.isVisionConfigured();
    ...
    try {
        ...
        if (!detectionResult.isScanned) {
            ...
            emitProgress(4, 'Google Vision OCR - SKIPPED (Native PDF)', { status: 'skipped', reason: 'Document is native/digital' });
        ...
        if (!visionConfig.configured) {
            ...
            emitProgress(2, 'PDF to Images - WARNING: Google Vision API not configured', {
                status: 'warning',
                message: visionConfig.details,
                fallback: 'Using Claude API directly for scanned document (may have lower accuracy)'
            });
            emitProgress(3, 'Image Preprocessing - SKIPPED (Vision API not configured)', { status: 'skipped' });
            emitProgress(4, 'Google Vision OCR - SKIPPED (Vision API not configured)', { 
                status: 'skipped',
                warning: 'Configure GOOGLE_APPLICATION_CREDENTIALS or GOOGLE_VISION_API_KEY for better OCR'
            });
            emitProgress(5, 'Text Reconstruction - SKIPPED (Vision API not configured)', { status: 'skipped' });
            emitProgress(6, 'Preparing payload for Claude API - Using scanned PDF directly (fallback mode)', {
                status: 'complete',
                contentType: 'pdf',
                sizeKB: Math.round(pdfBuffer.length / 1024),
                warning: 'Extraction accuracy may be reduced for scanned documents without Vision API'
            });
        ...
        try {
            ...
            if (!conversionResult.images || conversionResult.images.length === 0) {
                ...
                emitProgress(4, 'Google Vision OCR - SKIPPED (no images)', { status: 'skipped' });
            ...
            emitProgress(4, 'Google Vision OCR - SKIPPED (conversion failed)', { status: 'skipped' });
        ...
        // ========== STEP 4: Google Vision OCR ==========
        emitProgress(4, 'Google Vision OCR - Processing...', { status: 'in_progress' });
        ...
        try {
            ...
            if (!ocrResult.fullText || ocrResult.fullText.trim().length === 0) {
                emitProgress(4, `Google Vision OCR - WARNING: No text extracted (0 characters)`, {
                    status: 'warning',
                    confidence: 0,
                    pageCount: ocrResult.pageCount,
                    characterCount: 0,
                    cost: ocrResult.totalCost,
                    warning: 'Vision API returned no text. Document may be blank, image-only, or very poor quality.',
                    fallback: 'Falling back to Claude vision on PDF'
                });
                ...
                emitProgress(6, 'Preparing payload for Claude API - Using PDF directly (OCR returned no text)', {
                    status: 'complete',
                    contentType: 'pdf',
                    warning: 'Vision OCR extracted no text. Trying Claude vision on PDF directly.'
                });
                ...
                return {
                    ...
                    ocrWarning: 'Vision API returned no text',
                    processingSteps: steps,
                    totalProcessingTimeMs: Date.now() - startTime,
                    detectionDetails: detectionResult
                };
            ...
            emitProgress(4, `Google Vision OCR - confidence ${ocrResult.avgConfidence}%`, {
                status: 'complete',
            ...
            emitProgress(4, `Google Vision OCR - FAILED: ${error.message}`, {
                status: 'error',
                error: error.message
            });
            ...
            emitProgress(6, 'Preparing payload for Claude API - Using PDF directly (fallback)', {
                status: 'complete',
                contentType: 'pdf',
                warning: 'Vision OCR failed, using PDF directly'
            });
...
/**
 * Get combined API statistics (Vision + Claude)
 */
function getCombinedStats() {
    const visionStats = visionOcr.getVisionStats();
