 ...
 * "claude-sonnet-4-20250514" (unchanged from before). This module maps
 * those to the actual Bedrock model ID / cross-region inference profile
 * via BEDROCK_MODEL_ID (single override) or BEDROCK_MODEL_MAP (JSON, for
 * multiple Anthropic model strings -> different Bedrock IDs).
 *
 * IMPORTANT — verify before go-live: not every Claude model is available
 * as a direct on-demand model ID in every Bedrock region. ap-south-1
 * commonly requires a cross-region inference profile ID (e.g. prefixed
 * "apac." or "apac-anthropic."). Confirm the exact ID for your account
 * via `aws bedrock list-inference-profiles --region ap-south-1` and
 * `aws bedrock list-foundation-models --region ap-south-1` before
...
const DEFAULT_MODEL_MAP = {
    'claude-sonnet-4-20250514': process.env.BEDROCK_MODEL_ID || 'apac.anthropic.claude-sonnet-4-20250514-v1:0'
};
