...
const ALLOWED_DOMAINS = (process.env.ALLOWED_EMAIL_DOMAINS || '@acc.ltd,@axisfinance.in')
    .split(',')
...
// AFL employee-ID format, e.g. "AFL2631". ADFS currently releases the empId
// claim (not email), so users are whitelisted by empId until AFL releases
// the email claim. Configurable in case the format differs.
const EMPID_PATTERN = new RegExp(process.env.EMPID_PATTERN || '^afl\\d{3,}$', 'i');
