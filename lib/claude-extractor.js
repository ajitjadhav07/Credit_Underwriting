/**
 * Claude Extractor Module - Extracts financial data from documents using Claude API
 * Enhanced with Google Vision OCR pipeline for scanned documents
 */
...
function isOcrPipelineAvailable() {
    const visionConfigured = visionOcr ? visionOcr.isVisionConfigured() : { configured: false, method: 'none', details: 'Vision OCR module not loaded' };
...
async function checkVisionHealth() {
    if (!visionOcr || !visionOcr.checkVisionHealth) {
        return {
            working: false,
            configured: false,
            method: 'none',
    ...
    return await visionOcr.checkVisionHealth();
}
