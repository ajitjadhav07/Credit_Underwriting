...
/**
 * System prompt for CIBIL report extraction (Commercial or Individual/Consumer)
 */
const CIBIL_EXTRACTION_PROMPT = `You are an expert at analyzing Indian CIBIL / credit bureau reports
(Commercial CIR, CMR, or Individual Consumer reports from TransUnion CIBIL).

Extract:
1. Bureau score (CIBIL score for individuals, or CMR/CIBIL Rank for commercial reports)
2. All active/live credit facilities — lender name, facility type, sanctioned amount,
   current outstanding, EMI amount, account status (Active/Closed), DPD status
3. Days-Past-Due (DPD) history for the last 12 months — extract the MAXIMUM DPD seen
   across all accounts in the last 12 months
4. Write-offs, settlements, or suit-filed accounts
5. Willful defaulter status
6. Total number of active accounts and total outstanding across all accounts
7. Enquiry history (number of enquiries in last 6 months)
8. Report/pull date

CRITICAL - SCORE NORMALIZATION:
- Individual/Consumer CIBIL score: 300-900 range, return as-is in cibil_score
- Commercial CIBIL Rank (CMR): 1-10 scale where 1 is best. Convert to an approximate
  equivalent score for cibil_score using: score ≈ 900 - (rank-1)*60 (rank 1→900, rank 10→360)
  and ALSO return the raw rank in cibil_rank so both are available.

CRITICAL - LIVE LOANS:
List every account with status Active/Open/Standard as a live loan, including:
lender, facility_type (e.g. Term Loan, LAP, CC/OD, Personal Loan), sanctioned_amount,
current_outstanding, emi_amount, opened_date, account_status.
This is used for Balance-Transfer (BT) identification in loan eligibility — extract
EMI amounts precisely as they directly affect eligible loan calculations.
...
/**
 * Extract data from a user-uploaded CIBIL report (Commercial or Individual).
 * Replaces the CIBIL Middleware API call — the underwriter uploads the CIBIL
 * report PDF directly instead of the system calling CIBIL live.
 */
async function extractCIBIL(pdfBuffer, docName = 'CIBIL Report') {
    const base64Pdf = pdfBuffer.toString('base64');

    const userPrompt = `Extract credit bureau data from this CIBIL report (${docName}).

CRITICAL - INDIAN NUMBER FORMAT:
Indian documents use lakhs/crores comma grouping (e.g. 12,34,567 → 1234567).
Remove all commas but do not change digit count. Brackets mean negative.

Return a JSON object with this exact structure:
{
  "report_type": "commercial" | "individual" | null,
  "report_date": "<date bureau report was pulled, or null>",
  "borrower_name": "<name on report or null>",
  "cibil_score": <number 300-900, or null>,
  "cibil_rank": <number 1-10 for commercial CMR, or null>,
  "cmr_grade": "<e.g. CMR-3, or null>",
  "total_active_accounts": <number or null>,
  "total_outstanding": <number or null>,
  "max_dpd_last_12m": <number of days, 0 if none, or null>,
  "current_dpd": <number of days, 0 if none, or null>,
  "willful_defaulter": true | false | null,
  "suit_filed": true | false | null,
  "write_offs_settlements": <number of such accounts, 0 if none>,
  "enquiries_last_6m": <number or null>,
  "live_loans": [
    {
      "lender": "<bank/NBFC name>",
      "facility_type": "<Term Loan | LAP | CC/OD | Personal Loan | etc>",
      "sanctioned_amount": <number or null>,
      "current_outstanding": <number or null>,
      "emi_amount": <number or null>,
      "opened_date": "<date or null>",
...
Return ONLY the JSON object, no other text.`;
    ...
    try {
        const response = await anthropic.messages.create({
            ...
            max_tokens: 3072,
            temperature: 0,
            system: CIBIL_EXTRACTION_PROMPT,
            messages: [{
                role: "user",
                content: [
        ...
        const inputTokens = response.usage?.input_tokens || 0;
        const outputTokens = response.usage?.output_tokens || 0;
        console.log(`[API] CIBIL (${docName}) - Time: ${responseTime}s, Input: ${inputTokens}, Output: ${outputTokens}`);

        const responseText = response.content[0].text;
        const result = extractJsonFromResponse(responseText);

        // Normalize commercial CMR rank into an equivalent cibil_score if only
        // rank was extracted (some commercial reports show rank, not a 300-900 score)
        if ((result.cibil_score === null || result.cibil_score === undefined) && result.cibil_rank) {
            result.cibil_score = 900 - (result.cibil_rank - 1) * 60;
        }
        if (!Array.isArray(result.live_loans)) result.live_loans = [];

        addApiCallLog({
            callType: 'cibil_extraction',
            document: docName,
            documentSize: docSizeKB,
            model: 'claude-sonnet-4-20250514',
            promptSummary: `Extract CIBIL report data. Fields: score, live loans, DPD, defaults.`,
            responseTime,
            tokensUsed: { input: inputTokens, output: outputTokens, total: inputTokens + outputTokens },
            keyValues: { cibil_score: result.cibil_score, max_dpd_last_12m: result.max_dpd_last_12m, live_loans_count: result.live_loans.length },
            confidence: result.extraction_confidence || 'unknown',
            rateLimits: getRateLimits(),
            success: true
        });
        ...
        const responseTime = (Date.now() - startTime) / 1000;
        addApiCallLog({ callType: 'cibil_extraction', document: docName, responseTime, error: error.message, success: false });
        console.error('CIBIL extraction error:', error);
        throw new Error(`Failed to extract CIBIL report: ${error.message}`);
    }
}
