...
class BullQueueManager {
    ...
    async extractWithClaudeExtractor(buffer, docInfo, docName) {
        ...
        switch (type) {
            ...
            case 'cibil':
                ...
                break;
            case 'pennant_response':
            case 'mca_response':
            case 'gst_auth_response':
            case 'gst_return_response':
            case 'itr_response':
            case 'epfo_response':
            case 'bgv_response':
            case 'litigation_response':
            case 'fir_response':
            case 'peer_comparison_response':
            case 'trackwizz_response':
            case 'novel_response': {
                // Uploaded API response file — parse it (JSON preferred, PDF via Claude)
                // then store under extractedData.api_responses[api_key] so the
                // external-verification phase can use it and skip the live API call.
                const ingest = require('./api-response-ingest');
                const ingested = await ingest.ingestUploadedFile(docName, buffer);
                if (!extractedData.api_responses) extractedData.api_responses = {};
                const apiKey = type.replace('_response', '');
                if (ingested.json || ingested.text) {
                    extractedData.api_responses[apiKey] = {
                        source: 'uploaded_file',
                        fileName: docName,
                        api: ingested.api || apiKey,
                        hasCompleteData: ingested.hasCompleteData,
                        json: ingested.json,
                        text: ingested.text,
                        format: ingested.format,
                        uploadedAt: new Date().toISOString()
                    };
                    console.log(`${this.timestamp()} 📄 [WORKER] Stored uploaded ${apiKey} response (hasCompleteData: ${ingested.hasCompleteData})`);
                }
                result = extractedData.api_responses[apiKey] || null;
                break;
            }
            case 'property':
    ...
    async processAssessmentJob(job) {
        ...
        try {
            ...
            for (let i = 0; i < documents.length; i++) {
                ...
                try {
                    ...
                    if (docInfo.type === 'balance_sheet') {
                        ...
                        // Use actual month from the document key so the month field
                        // shows the real period (e.g. 'Jun 2025') not 'Monthly'.
                        const docKey = doc.id || doc.name || '';
                        const monthLabel = (function() {
                            const m = docKey.match(/bank_statement_([a-z]+)_(\d{4})/i);
                            if (m) return m[1].charAt(0).toUpperCase() + m[1].slice(1) + ' ' + m[2];
                            const m2 = docKey.match(/_(\d{4})_M(\d{2})/i);
                            if (m2) {
                                const months = ['','Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
                                return (months[parseInt(m2[2],10)] || 'M'+m2[2]) + ' ' + m2[1];
                            }
                            return 'Monthly';
                        })();
                        result = await claudeExtractor.extractBankStatement(docBuffer, monthLabel);
                        if (result) {
                            const bankData = result.bank_statement || result;
                            if (bankData) bankData.month = monthLabel;
            ...
            // HYBRID HELPER — returns the uploaded response if it is complete,
            // so the caller can skip the live API call. Returns null if no
            // complete upload exists (= should call API as normal).
            const uploadedResponse = (apiKey) => {
                const r = extractedData.api_responses?.[apiKey];
                if (r && r.hasCompleteData) {
                    console.log(`${this.timestamp()} 📄 [WORKER] Using uploaded ${apiKey} response — skipping live API call`);
                    return r.json || { _raw: r.text, source: 'uploaded_file' };
                }
                return null;
            };
            //                                        bank statement file
            ...
            if (externalApisManager || pennantClient) {
                ...
                try {
                    const uploadedPennant = uploadedResponse('pennant');
                    if (uploadedPennant) {
                        results.pennant = { success: true, source: 'uploaded_file', ...uploadedPennant };
                    } else if (pennantClient && finReference) {
                ...
                try {
                    const uploadedItr = uploadedResponse('itr');
                    if (uploadedItr) {
                        results.karza_itr = { success: true, source: 'uploaded_file', data: uploadedItr };
                    } else if (externalApisManager && pan && ack) {
                ...
                try {
                    ...
                    if (externalApisManager && statementDocs.length > 0) {
                        ...
                        // Step 3: Download the Excel analysis for each successfully uploaded statement
                        // Novel needs a short delay to process the uploaded file before it's ready
                        if (anySuccess) {
                            const successfulUploads = novelResults.filter(r => r.success && (r.doc_id || r.DOC));
                            if (successfulUploads.length > 0) {
                                console.log(`${this.timestamp()} ⏳ [NOVEL] Waiting 15s for Novel to process ${successfulUploads.length} uploaded statement(s)...`);
                                await this.sleep(15000);

                                const downloadResults = [];
                                for (const upload of successfulUploads) {
                                    const docId = upload.doc_id || upload.DOC;
                                    try {
                                        const dlResult = await externalApisManager.novelDownloadBankStatement({ docId, assessmentId });
                                        downloadResults.push({ ...dlResult, docId, sourceDoc: upload.doc });
                                        if (dlResult.success) {
                                            console.log(`${this.timestamp()} ✅ [NOVEL] Download OK for ${upload.doc} — ADB(12m): ₹${dlResult.avg_balance_last_12m}L`);
                                        } else {
                                            console.log(`${this.timestamp()} ❌ [NOVEL] Download failed for ${upload.doc}: ${dlResult.error}`);
                                        }
                                        await this.sleep(2000);
                                    } catch (dlErr) {
                                        downloadResults.push({ success: false, error: dlErr.message, docId, sourceDoc: upload.doc });
                                    }
                                }

                                results.novel_download = {
                                    success: downloadResults.some(r => r.success),
                                    total: downloadResults.length,
                                    results: downloadResults,
                                    // Convenience: aggregate ADB values across accounts for Banking Program
                                    // Each account's adb_lakhs feeds adb_account_1/2/3 in the CAM form
                                    accounts: downloadResults
                                        .filter(r => r.success && r.adb_lakhs != null)
                                        .map((r, i) => ({
                                            account_index: i + 1,
                                            source_doc: r.sourceDoc,
                                            doc_id: r.docId,
                                            adb_lakhs: r.adb_lakhs,
                                            avg_balance_last_12m: r.avg_balance_last_12m,
                                            avg_receipt_12m: r.avg_receipt_12m,
                                            inward_return_count: r.inward_return_count,
                                            avg_balance_last_6m: r.avg_balance_last_6m,
                                            avg_balance_last_3m: r.avg_balance_last_3m
                                        }))
                                };
                            }
                        }
            ...
            // 3.5 Auto-run CAM eligibility with defaults derived from Novel ADB +
            // Pennant loan amount + financials, so the assessment has a complete
            // CAM result without waiting for the user to click "Prefill + Run".
            try {
                const camEngine = require('./cam-eligibility');
                const camDefaults = camEngine.deriveDefaults(extractedData);
                // Overlay Pennant loan amount as proposed loan (rupees → lakhs)
                const penLoan = results?.pennant?.loanDetail || extractedData.pennant_data?.loanDetail || {};
                if (penLoan.financeAmount != null && camDefaults.proposed_loan == null) {
                    camDefaults.proposed_loan = parseFloat(penLoan.financeAmount) / 100000;
                }
                if (penLoan.tenureMonths != null) camDefaults.tenure_months = camDefaults.tenure_months || penLoan.tenureMonths;
                // Pick program: Banking if Novel ADB present, else Cash Profit
                const hasNovelAdb = camDefaults.adb_account_1 != null;
                const program = hasNovelAdb ? 'BANKING' : 'CASH_PROFIT';
                const camResult = camEngine.evaluate({ ...camDefaults, program });
                calculations.cam_eligibility = camResult;
                calculations.cam_input = camDefaults;
                console.log(`${this.timestamp()} ✅ [WORKER] CAM eligibility auto-run (${program}) — eligible ₹${camResult?.summary?.eligible_loan_amount ?? 'N/A'}L`);
                addApiLog('success', `CAM eligibility auto-computed using ${program} program`);
            } catch (camErr) {
                console.log(`${this.timestamp()} ⚠️ [WORKER] CAM auto-run skipped: ${camErr.message}`);
            }
            ...
            // 6.5 Trackwizz AML/RBI screening check — evaluate uploaded response
            // Rule: HitsCount > 0 OR ConfirmedHits === 'Yes' → hard Reject
            // regardless of credit score. OverallStatus 'AcceptedByTW' + no hits → Clean.
            let trackwizzResult = null;
            const twUploaded = extractedData.api_responses?.trackwizz;
            if (twUploaded && twUploaded.json) {
                const tw = twUploaded.json;
                const decData = tw.DecryptedData || tw;
                const overallStatus = decData.OverallStatus || '';
                let hitsCount = 0, confirmedHits = 'No', suggestedAction = 'Proceed';
                const customerResponses = decData.CustomerResponse || [];
                for (const cr of customerResponses) {
                    for (const pr of (cr.PurposeResponse || [])) {
                        const data = pr.Data || {};
                        hitsCount += (data.HitsCount || 0);
                        if (data.ConfirmedHits === 'Yes') confirmedHits = 'Yes';
                        if (data.SuggestedAction) suggestedAction = data.SuggestedAction;
                    }
                }
                const isAdverse = hitsCount > 0 || confirmedHits === 'Yes' ||
                                  overallStatus === 'RejectedByTW' || suggestedAction !== 'Proceed';
                trackwizzResult = {
                    source: 'uploaded_file',
                    overallStatus,
                    suggestedAction,
                    ...
                    confirmedHits,
                    isAdverse,
                    screeningOutcome: isAdverse
                        ? `ADVERSE: ${hitsCount} hit(s) detected in RBI/AML screening. SuggestedAction: ${suggestedAction}.`
                        : 'CLEAN: No adverse hits detected in RBI/AML screening.'
                };
                if (isAdverse) {
                    console.log(`${this.timestamp()} 🚨 [WORKER] Trackwizz ADVERSE HIT — HitsCount=${hitsCount}, ConfirmedHits=${confirmedHits}`);
                    addApiLog('error', `TRACKWIZZ SCREENING FAILED: ${trackwizzResult.screeningOutcome}`);
                } else {
                    console.log(`${this.timestamp()} ✅ [WORKER] Trackwizz clean — no adverse hits`);
                    addApiLog('success', `Trackwizz screening passed: ${trackwizzResult.screeningOutcome}`);
                }
            }

            // 7. Determine final status
            let status = this.determineStatus(creditScore, policyCompliance);
            // Trackwizz adverse hit overrides the credit-score-based decision
            if (trackwizzResult && trackwizzResult.isAdverse) {
                status = 'Rejected';
                addApiLog('error', 'Decision overridden to REJECTED due to Trackwizz adverse screening result');
            }
            ...
            const finalAssessment = {
                ...
                trackwizz_screening: trackwizzResult,
                external_findings: (function() {
                    try {
                        return require('./finding-narratives').buildAllFindings(extractedData.api_responses || extractedData.external_verification || {});
                    } catch (e) { return null; }
                })(),
