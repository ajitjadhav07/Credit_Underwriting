    ...
    async processAssessmentJob(job) {
        ...
        try {
            ...
            if (externalApisManager || pennantClient) {
                ...
                try {
                    if (externalApisManager && borrowerName && cin) {
                        results.cibil_commercial = await externalApisManager.fetchCommercialCIBIL({ borrowerName, cin, assessmentId });
                    } else {
                        results.cibil_commercial = { success: false, skipped: true, error: 'Borrower name or CIN/LLPIN not available' };
                    }
                } catch (err) {
                    results.cibil_commercial = { success: false, error: err.message };
                }

                try {
                    const statementDoc = documents.find(d => {
                        const t = this.parseDocumentId(d.id || d.name)?.type || '';
                        return t.includes('bank_statement');
                    });
                    if (externalApisManager && statementDoc) {
                        const fileBuffer = await this.loadDocumentFromS3(statementDoc.s3Key);
                        if (fileBuffer) {
                            results.novel_upload = await externalApisManager.novelUploadBankStatement({
                                fileBuffer, fileName: statementDoc.name, assessmentId
                            });
                        } else {
                            results.novel_upload = { success: false, error: 'Could not load bank statement from S3 for Novel upload' };
                        }
                    } else {
