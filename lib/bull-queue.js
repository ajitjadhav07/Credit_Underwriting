...
// External verification APIs (NSDL, CIBIL, Perfios, Karza) — routed via
// the centralized Middleware Gateway. Runs AFTER Claude extraction and
// BEFORE calculationEngine, per the finalized architecture.
...
class BullQueueManager {
    ...
    async extractWithClaudeExtractor(buffer, docInfo, docName) {
        ...
        switch (type) {
            ...
            case 'kyc':
                ...
                break;
            case 'cibil':
                // Use KYC extraction for CIBIL documents - extracts key fields
                result = await claudeExtractor.extractKYC(buffer, docName);
                break;
            case 'property':
    ...
    async processAssessmentJob(job) {
        ...
        try {
            ...
            // 2.6 External verification APIs — NSDL (PAN), CIBIL (credit score),
            // Perfios (bank analysis), Karza (ITR + GST). These run AFTER Claude's
            // extraction is complete and BEFORE the calculation engine, and are
            // routed entirely through the centralized Middleware Gateway.
            // Claude never sees this data — it only feeds calculationEngine.
            console.log(`${this.timestamp()} 🌐 [WORKER] Running external verification APIs (NSDL/CIBIL/Perfios/Karza)...`);
            ...
            this.emitProgress(assessmentId, {
                ...
                progress: 82,
                message: 'Verifying PAN, credit score, bank conduct, ITR and GST via Middleware...',
                apiLogs
            });

            if (externalApisManager) {
                try {
                    const externalVerification = await externalApisManager.runAllVerifications(extractedData, assessmentId);
                    extractedData.external_verification = externalVerification;

                    if (externalVerification.configured) {
                        const summary = [
                            externalVerification.nsdl?.success ? 'NSDL✓' : 'NSDL✗',
                            externalVerification.cibil?.success ? `CIBIL✓(${externalVerification.cibil.cibil_score ?? 'N/A'})` : 'CIBIL✗',
                            externalVerification.perfios?.success ? 'Perfios✓' : 'Perfios✗',
                            externalVerification.karza_itr?.success ? 'ITR✓' : 'ITR✗',
                            externalVerification.karza_gst?.success ? 'GST✓' : 'GST✗'
                        ].join(' ');
                        console.log(`${this.timestamp()} ✅ [WORKER] External verification complete: ${summary}`);
                        addApiLog('success', `External verification: ${summary}`);
                    } else {
                        console.log(`${this.timestamp()} ⚠️ [WORKER] External verification skipped: ${externalVerification.note}`);
                        addApiLog('warning', `External verification skipped: ${externalVerification.note}`);
                    }
                } catch (err) {
                    console.error(`${this.timestamp()} ❌ [WORKER] External verification failed:`, err.message);
                    addApiLog('error', `External verification failed: ${err.message}`);
                    extractedData.external_verification = { configured: false, error: err.message };
                }
            } else {
                console.log(`${this.timestamp()} ⚠️ [WORKER] External APIs Manager not available — skipping NSDL/CIBIL/Perfios/Karza`);
                extractedData.external_verification = { configured: false, note: 'external-apis-manager module not loaded' };
            }
            ...
            const finalAssessment = {
                ...
                type: 'actual',
                created_by: userId,  // Preserve who created this assessment
                ...
                all_extracted_data: extractedData, // Duplicate for compatibility
                calculations,
                credit_score: creditScore,
                policy_compliance: policyCompliance,
                recommended_limits: recommendedLimits,  // ← ADDED: Same as client-side
                legal_risk_assessment: legalRiskAssessment, // ← ADDED: Legal assessment
                api_stats: apiStats,
                api_logs: apiLogs,
                document_timings: documentTimings,  // ← ADDED: Per-document timing
